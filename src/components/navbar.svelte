<script lang="ts">
  export let printState: boolean = false;
</script>
<nav class="w-full p-2 bg-teal-300 ">
  <div class="grid grid-cols-3">
      <img src="qrcode.jpeg" class="h-12 self-center" alt="QR code leading to https://github.com/F-bh/QuRl"/> <!-- todo add QR Code leading to GitHub here-->
      <h1 class="text-center font-extrabold self-center text-2xl">
          QuRl
      </h1>
      <div class="grid justify-end text-right self-center">
        {#if !printState} <a class="p-1 hover:bg-teal-200 hover:rounded-2xl hover:text-slate-500" href="https://github.com/F-bh/QuRl">
          visit me on Github  🧑‍💻
        </a>
        {/if}
      </div>
    </div>
</nav>